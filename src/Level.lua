...
function Level:init()
    ...
    function beginContact(a, b, coll)
        ...
        if types['Player'] and types['Alien'] then
            ...
            if self.launchMarker then self.launchMarker.alien.collided = true end
        end
...
function Level:update(dt)
    ...
    if love.keyboard.wasPressed('space') and self.launchMarker.launched and not self.launchMarker.alien.collided then
        self.launchMarker:onSplit(self)
    end
    ...
    if self.launchMarker.launched then
        ...
        if self:isPlayerStill(self.launchMarker.alien) and 
            self:isPlayerStill(self.launchMarker.splitAliens[1]) and 
            self:isPlayerStill(self.launchMarker.splitAliens[2]) then
...
function Level:isPlayerStill(player)
    if player then 
