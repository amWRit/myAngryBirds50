...
function AlienLaunchMarker:init(world)
    ...
    -- to store splitted player aliens
    self.splitAliens = {}

    self.onSplit = function() end
end
...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        elseif love.mouse.wasReleased(1) and self.aiming then
            ...
            self.alien.collided = false

            -- we're no longer aiming
            self.aiming = false

            self.onSplit = function(level)
                local splitAlienTop = nil
                local splitAlienBottom = nil
                local velX, velY = self.alien.body:getLinearVelocity()

                --create split alien player
                splitAlienTop = Alien(self.world, 'round', self.alien.body:getX(), self.alien.body:getY()-25, 'Player', 15)                
                splitAlienTop.body:setLinearVelocity(velX + 10, velY - 20)
                splitAlienTop.fixture:setRestitution(0.4)
                splitAlienTop.body:setAngularDamping(1)   
                table.insert(self.splitAliens, splitAlienTop)

                --create another split alien player
                splitAlienBottom = Alien(self.world, 'round', self.alien.body:getX(), self.alien.body:getY()+25, 'Player', 15)
                splitAlienBottom.body:setLinearVelocity(velX + 10, velY + 20)
                splitAlienBottom.fixture:setRestitution(0.4)
                splitAlienBottom.body:setAngularDamping(1)  
                table.insert(self.splitAliens, splitAlienBottom)
            end
    ...
    if not self.launched then
        ...
        if self.aiming then
            ...
            for i = 1, 90 do
                ...
                love.graphics.setColor(255/255, 80/255, 255/255, (255 / 12)/255 * i)
